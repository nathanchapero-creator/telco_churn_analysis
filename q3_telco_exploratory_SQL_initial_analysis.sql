...
-- Churn reasons for customers on Offer E
SELECT
churn_category,
count(*)
FROM customer_status
...
-- Churn rate within by tenure 
SELECT
SUM(CASE WHEN churn_value = TRUE THEN 1 ELSE 0 END) AS churned_count,
count(*),
CASE
	WHEN tenure <= 3 THEN '0-3 Months'
	WHEN tenure >= 4 AND tenure <= 6 THEN '4-6 Months'
	WHEN tenure >= 7 AND tenure <= 12 THEN '7 - 12 Months'
	WHEN tenure >= 13 AND tenure <= 18 THEN '13 - 18 Months'
	ELSE 'More than 18 Months'
...
CASE
	...
	WHEN tenure <= 18 THEN '13 - 18 Months'
	WHEN tenure <= 24 THEN '19 - 24 Months'
